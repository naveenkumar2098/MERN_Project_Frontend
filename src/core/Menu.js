        ...
        <div>
            <ul className="nav nav-tabs bg-dark">
                ...
                {isAuthenticated() && isAuthenticated().user.role === 0 && (
                    <li className="nav-item">
                        <Link style ={currTab(history, "/user/dashboard")} className="nav-link" to="/user/dashboard">User DashBoard</Link>
                    </li>
                )}
                {isAuthenticated() && isAuthenticated().user.role === 1 && (
                    <li className="nav-item">
                        <Link style ={currTab(history, "/admin/dashboard")} className="nav-link" to="/admin/dashboard">Admin DashBoard</Link>
                    </li>
                )}
                {!isAuthenticated() && (
                    <Fragment>
